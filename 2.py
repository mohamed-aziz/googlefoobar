...
def solution(h, q):
    ...
    def build_binary_tree(height, current_value=1):
        if height == 0:
            return None

        left_child = build_binary_tree(height - 1, 2 * current_value)
        right_child = build_binary_tree(height - 1, 2 * current_value + 1)
        return TreeNode(current_value, left_child, right_child)
    ...
    def build_lookup_table(root, parent_label):
        if root is None:
            return

        m[root.value] = parent_label
        build_lookup_table(root.left, root.value)
        build_lookup_table(root.right, root.value)

    t = build_binary_tree(h)

    post_order_traversal(t)
    build_lookup_table(t, -1)
    return [m.get(c, -1) for c in q]
